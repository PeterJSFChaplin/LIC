...
class LayerViewer(gtk.Window):
    ...
    def save_file(self, btn, data=None) :
        ...
        config = {}
        config["layers"] = []
        for L in reversed(self.img.layers):
            config["layers"].append(traverse(L, ""))
        config["height"] = self.img.height
        config["width"] = self.img.width
        ...
        chooser.set_current_name(os.path.basename("%s_layout.json"%self.img.name))
        ...
        if filename:
            try:
                ...
                file_obj.write(json.dumps(config, indent=4, separators=(',', ': ')));
                file_obj.close()
                # for a file save ->
...
def traverse(layer, path):
    ...
    if hasattr(layer,"layers"):
        ...
        for L in reversed(layer.layers):
            config["layers"].append(traverse(L, source + "/"))
        config["type"] = "group"
    else:
        config["type"] = "single"
